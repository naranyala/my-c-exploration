...
void InitDemo(void) {
    ...
    box_static = gp2d_aabb_from_center(gp2d_v2(SCREEN_W - 200, 200), gp2d_v2_scale(box_size, 0.5f));
    
    circle_static = gp2d_circle_make(gp2d_v2(SCREEN_W - 200, SCREEN_H - 150), 60.0f);
}
...
void UpdateDemo(float dt) {
    ...
    // 2. Resolve collision and separation (gp2d_separate_circles is the key function here)
    // NOTE: This modifies the centers of both circles to push them apart.
...
void DrawDemo(void) {
    ...
    DrawText("TOP HALF: Circle-to-Circle Separation (Mouse moves Circle A, Right-Click moves Circle B)", 10, 40, 15, WHITE);
    DrawText("BOTTOM HALF: Raycasting (Mouse controls ray direction/origin)", 10, SCREEN_H/2 + 20, 15, WHITE);
    ...
    // Draw Circle A
    DrawCircle(circle_A.center.x, circle_A.center.y, circle_A.radius, Fade(color_A, 0.7f));
    DrawCircleLines(circle_A.center.x, circle_A.center.y, circle_A.radius, color_A);
    DrawText("Circle A (Mouse)", circle_A.center.x - 40, circle_A.center.y, 10, WHITE);

    // Draw Circle B
    DrawCircle(circle_B.center.x, circle_B.center.y, circle_B.radius, Fade(color_B, 0.7f));
    DrawCircleLines(circle_B.center.x, circle_B.center.y, circle_B.radius, color_B);
    DrawText("Circle B (Hold R-Click)", circle_B.center.x - 60, circle_B.center.y, 10, WHITE);
    
    // Draw collision information
    if (col_res.hit) {
        DrawText(TextFormat("COLLISION! Depth: %.2f", col_res.depth), 10, 70, 20, RED);
        ...
        gp2d_vec2 normal_end = gp2d_v2_add(midpoint, gp2d_v2_scale(col_res.normal, 80.0f));
        DrawLineEx((Vector2){midpoint.x, midpoint.y}, (Vector2){normal_end.x, normal_end.y}, 3, YELLOW);
    ...
    // Draw AABB
    DrawRectangleLines(box_static.min.x, box_static.min.y, 
                       box_static.max.x - box_static.min.x, 
                       box_static.max.y - box_static.min.y, 
                       res_aabb.hit ? RED : GRAY);

    // Draw Ray
    Color ray1_color = res_aabb.hit ? RED : LIME;
    gp2d_vec2 ray1_end = res_aabb.hit ? res_aabb.point : gp2d_v2_add(ray1_origin, gp2d_v2_scale(ray1_dir, 1000.0f));
    ...
    if (res_aabb.hit) {
        ...
        gp2d_vec2 normal_end = gp2d_v2_add(res_aabb.point, gp2d_v2_scale(res_aabb.normal, 50.0f));
        DrawLineEx((Vector2){res_aabb.point.x, res_aabb.point.y}, (Vector2){normal_end.x, normal_end.y}, 3, PURPLE);
    ...
    // Draw Static Circle
    DrawCircle(circle_static.center.x, circle_static.center.y, circle_static.radius, 
               Fade(res_circle.hit ? GOLD : BLUE, 0.3f));
    DrawCircleLines(circle_static.center.x, circle_static.center.y, circle_static.radius, 
                    res_circle.hit ? GOLD : BLUE);
    
    // Draw Ray
    Color ray2_color = res_circle.hit ? GOLD : LIME;
    gp2d_vec2 ray2_end = res_circle.hit ? res_circle.point : gp2d_v2_add(ray2_origin, gp2d_v2_scale(ray2_dir, 500.0f));
    DrawLineEx((Vector2){ray2_origin.x, ray2_origin.y}, (Vector2){ray2_end.x, ray2_end.y}, 2, ray2_color);

    // Draw hit point and normal
    if (res_circle.hit) {
        DrawCircleV((Vector2){res_circle.point.x, res_circle.point.y}, 8, GOLD);
        gp2d_vec2 normal_end = gp2d_v2_add(res_circle.point, gp2d_v2_scale(res_circle.normal, 50.0f));
        DrawLineEx((Vector2){res_circle.point.x, res_circle.point.y}, (Vector2){normal_end.x, normal_end.y}, 3, GOLD);
