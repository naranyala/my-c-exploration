...
// -------- Basic types --------
typedef struct { float x, y; } anim_vec2;
typedef struct { float r, g, b, a; } anim_rgba;
...
typedef struct {
    ...
    union {
        struct { float a; } scalar;
        struct { anim_vec2 va; } vec2;
        struct { anim_rgba ca; } color;
    } start;
    union {
        struct { float b; } scalar;
        struct { anim_vec2 vb; } vec2;
        struct { anim_rgba cb; } color;
    } end;
} anim_key;
...
typedef enum { ANIM_PLAY_ONCE, ANIM_PLAY_LOOP, ANIM_PLAY_PINGPONG } anim_play_mode;
...
ANIM2D_DEF int  anim_track_add_scalar(anim_track* trk, float t0, float t1, float a, float b, anim_ease_fn f);
ANIM2D_DEF int  anim_track_add_vec2  (anim_track* trk, float t0, float t1, anim_vec2 a, anim_vec2 b, anim_ease_fn f);
ANIM2D_DEF int  anim_track_add_color (anim_track* trk, float t0, float t1, anim_rgba a, anim_rgba b, anim_ease_fn f);

ANIM2D_DEF void anim_clip_reset(anim_clip* clip);
ANIM2D_DEF void anim_clip_config(anim_clip* clip, float duration, float speed, float delay, anim_play_mode mode);
...
ANIM2D_DEF void anim_track_from_path(anim_track* trk, anim_path_fn fn, void* user, float duration, int samples, anim_ease_fn ease);

// -------- Rendering callback pattern --------
// You own rendering. Example draw callback receiving evaluated values per track.
...
ANIM2D_DEF float anim_lerp(float a,float b,float t){ return a + (b-a)*t; }
ANIM2D_DEF anim_vec2 anim_lerp2(anim_vec2 a, anim_vec2 b, float t){
    anim_vec2 r = { anim_lerp(a.x,b.x,t), anim_lerp(a.y,b.y,t) }; return r;
}
ANIM2D_DEF anim_rgba anim_lerpc(anim_rgba a, anim_rgba b, float t){
    anim_rgba r = { anim_lerp(a.r,b.r,t), anim_lerp(a.g,b.g,t), anim_lerp(a.b,b.b,t), anim_lerp(a.a,b.a,t) }; return r;
}

// Easing
ANIM2D_DEF float anim_ease_linear(float t){ return anim_clamp01(t); }
ANIM2D_DEF float anim_ease_smooth(float t){ t = anim_clamp01(t); return t*t*(3.f - 2.f*t); }
ANIM2D_DEF float anim_ease_inquad(float t){ t = anim_clamp01(t); return t*t; }
ANIM2D_DEF float anim_ease_outquad(float t){ t = anim_clamp01(t); return 1.f - (1.f - t)*(1.f - t); }
ANIM2D_DEF float anim_ease_inoutquad(float t){
    t = anim_clamp01(t); return t < 0.5f ? 2.f*t*t : 1.f - powf(-2.f*t + 2.f, 2.f)/2.f;
}

ANIM2D_DEF void anim_track_reset(anim_track* trk){ trk->count = 0; }
ANIM2D_DEF int anim_track_add_scalar(anim_track* trk, float t0,float t1,float a,float b,anim_ease_fn f){
    if(trk->count >= ANIM_MAX_KEYS) return 0;
    anim_key k;
    k.t0=t0; k.t1=t1; k.ease=f?f:anim_ease_linear; k.kind=ANIM_VAL_SCALAR;
    k.start.scalar.a=a; k.end.scalar.b=b;
    trk->keys[trk->count++] = k; return 1;
}
ANIM2D_DEF int anim_track_add_vec2(anim_track* trk, float t0,float t1, anim_vec2 a, anim_vec2 b, anim_ease_fn f){
    if(trk->count >= ANIM_MAX_KEYS) return 0;
    anim_key k;
    k.t0=t0; k.t1=t1; k.ease=f?f:anim_ease_linear; k.kind=ANIM_VAL_VEC2;
    k.start.vec2.va=a; k.end.vec2.vb=b;
    trk->keys[trk->count++] = k; return 1;
}
ANIM2D_DEF int anim_track_add_color(anim_track* trk, float t0,float t1, anim_rgba a, anim_rgba b, anim_ease_fn f){
    if(trk->count >= ANIM_MAX_KEYS) return 0;
    anim_key k;
    k.t0=t0; k.t1=t1; k.ease=f?f:anim_ease_linear; k.kind=ANIM_VAL_COLOR;
    k.start.color.ca=a; k.end.color.cb=b;
    trk->keys[trk->count++] = k; return 1;
}

ANIM2D_DEF void anim_clip_reset(anim_clip* clip){
    clip->duration = 0; clip->time = 0; clip->speed = 1; clip->delay = 0; clip->mode = ANIM_PLAY_ONCE; clip->reverse = 0;
    clip->track_count = 0; memset(clip->tracks, 0, sizeof(clip->tracks));
}
ANIM2D_DEF void anim_clip_config(anim_clip* clip, float duration,float speed,float delay,anim_play_mode mode){
    clip->duration = duration; clip->speed = speed; clip->delay = delay; clip->mode = mode;
}
ANIM2D_DEF void anim_clip_add(anim_clip* clip, anim_track trk){
    if(clip->track_count < ANIM_MAX_TRACKS) clip->tracks[clip->track_count++] = trk;
}
...
static float anim_clip_wrap_time(const anim_clip* c, float t){
    ...
    if(c->mode == ANIM_PLAY_ONCE){
        ...
        float m = fmodf(t, c->duration); return m < 0 ? m + c->duration : m;
    } else { // ping-pong
        if(c->duration <= 0) return 0;
        float cycle = c->duration * 2.f;
        float m = fmodf(t, cycle); if(m < 0) m += cycle;
...
static void anim_eval_key(const anim_key* k, float t, anim_eval* out){
    ...
    u = anim_clamp01(u); u = k->ease ? k->ease(u) : u;
    out->kind = k->kind; out->active = 1;
...
ANIM2D_DEF anim_vec2 anim_path_lissajous(float t, void* user){
    ...
    anim_vec2 v = { p->A * sinf(p->a * t + p->phi),
                    p->B * sinf(p->b * t) };
    return v;
}
ANIM2D_DEF void anim_track_from_path(anim_track* trk, anim_path_fn fn, void* user, float duration, int samples, anim_ease_fn ease){
