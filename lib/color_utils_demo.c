...
// Slider helper
float DrawSlider(int posX, int posY, int width, int height, float value, Color color, const char *label) {
    ...
    if (IsMouseButtonDown(MOUSE_LEFT_BUTTON)) {
        ...
        if (mouse.x >= posX && mouse.x <= posX + width &&
            mouse.y >= posY && mouse.y <= posY + height) {
...
int main(void) {
    ...
    while (!WindowShouldClose()) {
        ...
        RGB rgb = { (unsigned char)(rVal * 255),
                    (unsigned char)(gVal * 255),
        ...
        // Manipulation slider
        lightFactor = DrawSlider(50, 500, 750, 20, lightFactor, DARKBLUE, "Lighten/Darken");
        RGB manipulated = (lightFactor >= 0.5)
