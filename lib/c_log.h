...
// Get the base filename (useful for POSIX/UNIX, less so for simple C, but good practice)
#define C_LOG_GET_FILENAME(file) (strrchr(file, '/') ? strrchr(file, '/') + 1 : (strrchr(file, '\\') ? strrchr(file, '\\') + 1 : file))

// Core logging function (internal)
void c_log_print(const char *level_str, const char *file, int line, const char *fmt, ...);

// --- Primary API Macros (Conditional on LOG_LEVEL) ---

#if C_LOG_LEVEL >= C_LOG_LEVEL_ERROR
#define C_LOG_ERROR(fmt, ...) \
    c_log_print("ERROR", C_LOG_GET_FILENAME(__FILE__), __LINE__, fmt, ##__VA_ARGS__)
#else
...
#define C_LOG_WARN(fmt, ...) \
    c_log_print("WARN ", C_LOG_GET_FILENAME(__FILE__), __LINE__, fmt, ##__VA_ARGS__)
#else
...
#define C_LOG_INFO(fmt, ...) \
    c_log_print("INFO ", C_LOG_GET_FILENAME(__FILE__), __LINE__, fmt, ##__VA_ARGS__)
#else
...
#define C_LOG_DEBUG(fmt, ...) \
    c_log_print("DEBUG", C_LOG_GET_FILENAME(__FILE__), __LINE__, fmt, ##__VA_ARGS__)
#else
...
void c_log_print(const char *level_str, const char *file, int line, const char *fmt, ...) {
    ...
    fprintf(level_str[0] == 'E' ? stderr : stdout, 
            "[%s] %s %s:%d | ", time_str, level_str, file, line);
