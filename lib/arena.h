/* arena.h v1.2 - Public Domain / MIT No Attribution - 2025
   Ultra-fast bump allocator with nested scopes, debug fill, and optional leak
   tracking. Faster than malloc/free, perfect for games, tools, parsers,
   compilers.
...
static inline void *arena_alloc(arena_t *a, size_t size) {
  ...
  if (a->curr_offset + size > a->buf_len) {
    ...
    while (new_cap < a->curr_offset + size)
      new_cap *= 2;
...
static inline size_t arena_size(const arena_t *a) { return a->curr_offset; }

static inline size_t arena_capacity(const arena_t *a) { return a->buf_len; }

// Helper macros
#define ARENA_SCOPE(a)                                                         \
  for (int _i = 1, _n = 0; _i && (_n = 1, 1); _i = 0, arena_end(_scope, (a)))
#define arena_scope_t                                                          \
  arena_begin(&(a));                                                           \
  arena_scope_t _scope =
