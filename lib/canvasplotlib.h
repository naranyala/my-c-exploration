...
static float scale_y(PlotContext *ctx, float y) {
  ...
  return ctx->area.y + ctx->area.height -
         (y - ctx->ymin) / range * ctx->area.height;
}

static void PlotInit(PlotContext *ctx, int w, int h) {
  *ctx = (PlotContext){.xmin = -10,
                       .xmax = 10,
                       .ymin = -10,
                       .ymax = 10,
                       .width = w,
...
static void PlotDrawGridAndAxes(PlotContext *ctx) {
  ...
  if (ctx->showGrid) {
    ...
    for (int i = (int)ctx->xmin; i <= (int)ctx->xmax; i++) {
      ...
      DrawLine(x, ctx->area.y, x, ctx->area.y + ctx->area.height,
               GetColor(0xE0E0E0FF));
    }
    for (int i = (int)ctx->ymin; i <= (int)ctx->ymax; i++) {
      ...
      DrawLine(ctx->area.x, y, ctx->area.x + ctx->area.width, y,
               GetColor(0xE0E0E0FF));
    }
  }
  ...
  if (zero_x >= ctx->area.x && zero_x <= ctx->area.x + ctx->area.width)
    DrawLine(zero_x, ctx->area.y, zero_x, ctx->area.y + ctx->area.height,
             DARKGRAY);
  if (zero_y >= ctx->area.y && zero_y <= ctx->area.y + ctx->area.height)
    DrawLine(ctx->area.x, zero_y, ctx->area.x + ctx->area.width, zero_y,
             DARKGRAY);

  DrawRectangleLinesEx(ctx->area, 2, DARKGRAY);
}

static void PlotFunction(PlotContext *ctx, Color color, float t_min,
                         float t_max, int steps, float (*f_x)(float),
                         float (*f_y)(float), float time) {
...
// Pie Chart
static void DrawPieChart(Vector2 center, float radius, ChartData *data,
                         int count) {
  float total = 0;
  ...
  for (int i = 0; i < count; i++) {
    ...
    Vector2 labelPos = {center.x + cosf(midAngle) * labelRadius,
...
static void DrawBarChart(Rectangle area, ChartData *data, int count) {
  ...
  DrawLineEx((Vector2){area.x, area.y}, (Vector2){area.x, area.y + area.height},
             2, DARKGRAY);
  ...
  for (int i = 0; i < count; i++) {
    ...
    DrawText(data[i].label, x + barWidth / 2 - labelWidth / 2,
             area.y + area.height + 10, 16, DARKGRAY);
  }
}
...
static void DrawLineChart(Rectangle area, ChartData *data, int count) {
  ...
  for (int i = 0; i < count - 1; i++) {
    ...
    float y2 =
        area.y + area.height - (data[i + 1].value / maxValue) * area.height;
  ...
  for (int i = 0; i < count; i++) {
    ...
    // Draw label below
    int labelWidth = MeasureText(data[i].label, 16);
    DrawText(data[i].label, x - labelWidth / 2, area.y + area.height + 10, 16,
             DARKGRAY);
  }
}
