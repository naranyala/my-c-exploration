...
#define dynarray(T) struct { T *data; size_t len, cap; }
...
#define _dynarray_grow(arr, need) \
    do { \
        if ((arr)->len + (need) >= (arr)->cap) { \
            ...
            while (new_cap < (arr)->len + (need)) new_cap *= 2; \
...
#define dynarray_first(arr) ((arr)->len > 0 ? (arr)->data[0] : (typeof(*(arr)->data)){0})
#define dynarray_last(arr) ((arr)->len > 0 ? (arr)->data[(arr)->len - 1] : (typeof(*(arr)->data)){0})
...
#define dynarray_append_array(arr, src_array, count) \
    do { \
        ...
        memcpy((arr)->data + (arr)->len, (src_array), _count * sizeof(*(arr)->data)); \
        (arr)->len += _count; \
    } while (0)
