...
void init_bounce_animation(anim_clip* clip) {
    ...
    anim_track_add_scalar(&scale_track, 1.0f, 1.2f, 1.4f, 1.0f, anim_ease_outquad);
...
void init_circle_animation(anim_clip* clip) {
    ...
    anim_track_from_path(&circle_track, anim_path_circle, &params, 3.0f, 60, anim_ease_linear);
...
void init_color_animation(anim_clip* clip) {
    ...
    anim_track_add_color(&color_track, 1.0f, 1.5f, green, yellow, anim_ease_smooth);
...
void init_lissajous_animation(anim_clip* clip) {
    ...
    anim_track_from_path(&lissa_track, anim_path_lissajous, &params, 6.28f, 100, anim_ease_linear);
...
void init_rotate_animation(anim_clip* clip) {
    ...
    anim_track_add_scalar(&rot_track, 0.0f, 2.0f, 0.0f, 360.0f, anim_ease_inoutquad);
    anim_track_add_scalar(&rot_track, 2.0f, 4.0f, 360.0f, 0.0f, anim_ease_inoutquad);
...
int main(void) {
    ...
    while (!WindowShouldClose()) {
        ...
        // 3. Color pulse animation
        {
            ...
            if (eval.active) {
                Color c = {
                    (unsigned char)(eval.value.c.r * 255),
        ...
        // Draw animation info
        DrawText(TextFormat("Bounce Time: %.2f", state.bounce_clip.time), 20, SCREEN_H - 100, 16, GRAY);
        DrawText(TextFormat("Circle Time: %.2f", state.circle_clip.time), 20, SCREEN_H - 75, 16, GRAY);
        DrawText(TextFormat("Color Time: %.2f", state.color_clip.time), 20, SCREEN_H - 50, 16, GRAY);
        DrawText(TextFormat("Lissajous Time: %.2f", state.lissajous_clip.time), 20, SCREEN_H - 25, 16, GRAY);
